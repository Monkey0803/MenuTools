...
- Shared-folder sync covers "pinned items + snippets": deleting a pinned item, snippet, or group propagates as a deletion to other devices (tombstones are kept for 30 days), and snippets move to the default group when their group is deleted. Snippet renames and favorites resolve in favour of the newer update. **Unpinning is not propagated** and must be repeated on each device.
