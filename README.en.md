...
1. Update `CFBundleShortVersionString` and `CFBundleVersion` in `Resources/Info.plist`, commit, and make sure the working tree is clean (the script enforces this).
...
3. Create a GitHub Release with a tag such as `v1.1.0` and attach
   - `dist/MenuTools-<version>.zip`
   - `dist/MenuTools-<version>.dmg`
   - `dist/appcast.xml` (must keep this filename: `SUFeedURL` points to `releases/latest/download/appcast.xml`, otherwise clients never see the update)
4. Put the printed SHA-256 values and a signing note into the release body (see the v1.0.4 release for the structure).
