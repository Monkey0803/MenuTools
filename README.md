...
| 🪟 窗口管理 | 60 种布局（含 Loop 式边缘收纳）、边缘吸附与落点预览（含触觉反馈与生长动画）、可选的精细吸附区域、连按同族循环、连按半屏跨显示器、拖出已吸附窗口恢复原尺寸、固定尺寸预设（可绑全局快捷键、快速面板一键套用）、按窗口标题过滤的应用规则、多窗口排列、按应用尺寸记忆，以及 `menutools://` 链接触发 |
