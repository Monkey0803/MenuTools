...
若误点拒绝，可在 **系统设置 → 隐私与安全性** 中重新开启。通知被拒绝后在 **系统设置 → 通知 → MenuTools** 重新允许；网络流量设置页会显示当前通知授权状态，并在被拒时提供直达系统设置的入口。静音、防止锁屏、夜览、剪贴板历史记录和清理类功能无需任何权限（剪贴板自动粘贴除外）。
...
| 窗口管理 | Accessibility API 调整窗口位置、尺寸和显示器；NSEvent 全局拖拽监听驱动边缘吸附与落点预览；连按循环、按应用尺寸记忆、布局预设和应用规则持久化；设置页与快速面板按形状族分组布局并提供搜索 | 窗口管理 |
...
- 网络流量依赖系统 `/usr/bin/nettop` 的进程级采样：只能按进程/App 汇总，无法把同一进程的流量拆到各个远端目标（连接明细只展示当前连接与端点）；网卡筛选走的是 nettop 的接口类别（external、wifi、wired、awdl、expensive、loopback），不是物理网卡名，VPN（utun）等隧道不会单列
- 网络流量的历史趋势、排名和今日/本月统计来自本机累积采样：刚开启时没有历史数据，需要运行一段时间才会出现；系统大版本升级后若 nettop 输出字段变化，可跑 `Scripts/test_network_traffic.swift` 回归
- 高流量与月度额度提醒需要通知权限；被拒绝时提醒不会送达，网络流量设置页会显示状态并提供跳转系统设置的按钮
